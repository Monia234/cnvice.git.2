...
TRV=function(Nj,repeticoes,filename_F0='TRV_F0.RData',filename_FS='TRV_Fs.RData',apagarRData=TRUE){
	p.repF0=CNVice.rep(Nj,repeticoes,FALSE,filename_F0,FALSE) #calls "Estimated allele frequencies" function (allele probabilities)
	resultadoF0=resumeCnvice(p.repF0,7,7)

	p.repFs=CNVice.rep(Nj,repeticoes,TRUE,filename_FS,FALSE) #calls "Estimated allele frequencies" function (allele probabilities)
	resultadoFs=resumeCnvice(p.repFs,7,7)
	...
	return(list("trv"=d1,"rejeita"=(d1>2.42075),"resultadoF0"=resultadoF0,"resultadoFs"=resultadoFs))
}
...
#apagarRData: delete backup file? (TRUE or FALSE)
 ...
 #barplot(rev(Nj),horiz=TRUE,main="Observado",names.arg=(length(Nj)-1):0)
  ...
  if(estimar_F){
	trv=TRV(Nj,rept,filename_FS=filename_backup,apagarRData=apagarRData)
  }else{
	trv=TRV(Nj,rept,filename_F0=filename_backup,apagarRData=apagarRData)
  }
